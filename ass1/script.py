...
class TMatrix:
    ...
    def mult(self, other_matrix):
        ...
        if len(other_matrix) == self.num_col: # check if dimension match
            for i in range(self.num_row): # iterate through rows of the first matrix
                new_row = []
                for j in range(len(other_matrix[0])): # iterate through columns of the second matrix
                    value = 0 # calculate value at i, j
                    # iterate through columns of the first and rows of the second matrix
                    for q in range(self.num_col):
...
identity_mat = TMatrix() #identity matrix
...
print(make_trans_mat(-4,0,2).matrix)
print(make_trans_mat(-2,0,4).mult(make_rot_mat(180,'y').matrix))
